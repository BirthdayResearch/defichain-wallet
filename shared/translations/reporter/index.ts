...
function checkTranslations(
  ...
  missingTranslations: MissingLanguageReport
): MissingLanguageReport {
  const localeToExclude = ["en", "de"];
  const languages = getAppLanguages()
    .map((language) => language.locale)
    .filter((locale) => !localeToExclude.includes(locale));
  let totalCount = 0;
  ...
  baseTranslation.forEach((labels, screenName) => {
    ...
    languages.forEach((language) => {
      ...
      const languageTranslations = missingTranslations.missingLanguageItems[
        language
      ] ?? { missingCount: 0, labels: {}, totalCount };
      const translationFile = langItem[screenName];
      if (translationFile === null || translationFile === undefined) {
        languageTranslations.labels[screenName] = labels.map((label) =>
          decodeLabel(label)
        );
        ...
        labels.forEach((baseLabel) => {
          if (
            translationFile[baseLabel] === null ||
            translationFile[baseLabel] === undefined
          ) {
            languageTranslations.labels[screenName] =
              languageTranslations.labels[screenName] ?? [];
            languageTranslations.labels[screenName].push(
              decodeLabel(baseLabel)
            );
      ...
      languageTranslations.allLabels = JSON.stringify(
        languageTranslations.labels,
        null,
        4
      );
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(missingTranslations, null, 4)
  );
  return missingTranslations;
}

export async function findMissingTranslations(): Promise<MissingLanguageReport> {
  const missingTranslations: MissingLanguageReport = {
