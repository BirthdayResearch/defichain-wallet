...
interface BottomSheetTokenProps {
...
interface BottomSheetTokenListProps extends BottomSheetTokenProps {
  headerLabel: string
...
const SheetToken = ({
  ...
  vault,
  tokenType
}: BottomSheetTokenProps & {item: CollateralItem | BottomSheetToken}): JSX.Element => {
  ...
  const [activePrice, setActivePrice] = useState(new BigNumber(''))
  ...
  const getTokenPriceDetails = async (): Promise<void> => {
   const activePrice = tokenType === TokenType.CollateralItem
    ...
    : await getTokenPrice(item.token.id, new BigNumber('1'))
