...
import { SymbolIcon, SymbolIconV2 } from "@components/SymbolIcon";
...
export function TokenIconGroupV2(props: TokenIconGroupProps): JSX.Element {
  const additionalIcon = BigNumber.max(
    props.symbols?.length - props.maxIconToDisplay,
    0
  );
      ...
      {props.symbols?.map((symbol, index): JSX.Element | null => {
        if (index < props.maxIconToDisplay) {
          return (
            ...
            >
              <SymbolIconV2 key={symbol} symbol={symbol} />
            </View>
          );
        }
        return null;
