...
const Tabs = memo((props: TabsProps): JSX.Element => {
        ...
        {props.tabSections.map((tab) => {
          ...
          return (
            <View key={tab.id} style={tailwind("w-2/4 flex items-center")}>
              <ThemedTouchableOpacity
                light={tailwind("border-b-2", {
                  "border-primary-500": isActive,
                })}
                dark={tailwind("border-b-2", {
                  "border-darkprimary-500": isActive,
                })}
  ...
  function ScrollableTabs(): JSX.Element {
    return (
  ...
  return (
    <ThemedText
      light={tailwind({
        ...
        "text-gray-500": !props.isActive && !props.tab.disabled,
        "text-black": props.isActive,
      })}
      dark={tailwind({
        "text-gray-700": props.tab.disabled,
        "text-gray-400": !props.isActive && !props.tab.disabled,
        "text-white": props.isActive,
      })}
      style={tailwind("text-base pb-3 text-center", {
        "font-semibold": props.isActive,
      })}
    >
