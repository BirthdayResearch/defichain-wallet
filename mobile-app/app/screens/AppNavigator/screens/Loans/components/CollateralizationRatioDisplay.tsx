...
export function CollateralizationRatioDisplay(
  ...
  const normalizedNextRatio = new BigNumber(props.nextCollateralizationRatio)
    .dividedBy(maxRatio)
    .multipliedBy(100);
  ...
  return (
    ...
    >
      ...
      />
      <HealthBar
        normalizedColRatio={normalizedColRatio}
        normalizedNextRatio={normalizedNextRatio}
      />
      <ColorScale
...
function CollateralizationRatioText(props: {
  colRatio: string;
  minColRatio: string;
  totalLoanAmount: string;
  testID?: string;
  ...
  return (
    <View style={tailwind("flex-row justify-between")}>
      <View style={tailwind("items-center flex-row")}>
        <ThemedText style={tailwind("text-sm mr-0.5")}>
          {translate(
...
function MinAndNextRatioText(props: {
  minColRatio: string;
  nextColRatio: string;
  testID: string;
}): JSX.Element {
      ...
      {props.normalizedColRatio.isGreaterThanOrEqualTo(0) && (
        <>
          <ThemedView
            ...
            style={[
              tailwind("w-px h-4 absolute bottom-0"),
              {
                left: `${BigNumber.min(
                  props.normalizedColRatio.multipliedBy(100),
                  99.7
                ).toFixed(2)}%`,
              },
            ]}
          ...
          <View
            style={[
              ...
              {
                left: `${BigNumber.min(props.normalizedNextRatio, 99.7).toFixed(
                  2
                )}%`,
              },
            ]}
  ...
  return (
    <View style={tailwind("flex flex-row mt-1")}>
      <ThemedView
        ...
        style={[
          tailwind("h-1"),
          { width: `${props.normalizedLiquidatedThreshold.toFixed(2)}%` },
        ]}
      />
      <ThemedView
