...
export function CollateralizationRatioDisplayV2(
  ...
  // TODO (Harsh) need to check condition when nextCollateralizationRatio goes to -ve and more than maxRatio
  const normalizedNextFactor = new BigNumber(
    props.nextCollateralizationRatio
  ).dividedBy(maxRatio);
  const stats = useCollateralRatioStats({
    colRatio: new BigNumber(props.nextCollateralizationRatio),
  ...
  return (
    ...
    >
      <View style={tailwind("flex-row justify-between mb-3")}>
        ...
        {new BigNumber(props.nextCollateralizationRatio).isLessThan(0) ? (
          <ThemedTextV2
          ...
          <NumberFormat
            value={new BigNumber(props.nextCollateralizationRatio).toFixed(2)}
            decimalScale={2}
            thousandSeparator
            displayType="text"
            suffix="%"
