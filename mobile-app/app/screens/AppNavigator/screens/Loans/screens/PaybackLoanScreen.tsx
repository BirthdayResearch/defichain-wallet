...
export function PaybackLoanScreen ({
  ...
  useEffect(() => {
    return setIsValidInput(new BigNumber(amountToPay).lte(0) || amountToPay === '')
  }, [amountToPay])
    ...
    <ThemedScrollView contentContainerStyle={tailwind('pb-8 pt-2')}>
      ...
      <View style={tailwind('my-2 px-4')}>
        <WalletTextInput
          ...
          valid={hasSufficientPaymentTokenBalance || isValidInput}
          {...(!hasSufficientPaymentTokenBalance && !isValidInput && {
            inlineText: {
              type: 'error',
