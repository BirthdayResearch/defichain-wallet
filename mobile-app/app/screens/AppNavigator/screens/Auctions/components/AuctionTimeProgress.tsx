...
export const AuctionTimeProgress = memo(
  (props: AuctionTimeProgressProps): JSX.Element => {
    ...
    const normalizedBlocks = new BigNumber(blocksRemaining)
      .dividedBy(blocksPerAuction)
      .toNumber();
      ...
      <>
        <View style={tailwind("flex-row w-full justify-between mb-2")}>
          ...
          <View style={tailwind("flex flex-row flex-1 justify-end flex-wrap")}>
            ...
            >
              {timeRemaining !== "" &&
                translate("components/AuctionTimeProgress", "~{{time}} left", {
                  time: timeRemaining,
                })}{" "}
              (
              {translate("components/AuctionTimeProgress", "{{block}} blks", {
