  ...
  return (
    <ThemedFlatList
      ...
      contentContainerStyle={[
        tailwind("p-4 pb-8"),
        tailwind({
          "pb-44": Platform.OS !== "android" && Platform.OS !== "ios",
        }),
      ]}
...
function BidHistoryItem(props: BidHistoryItemProps): JSX.Element {
  ...
  return (
    <ThemedView
      light={tailwind([
        "border-gray-200",
        { "bg-white": props.isLatestBid, "bg-gray-50": !props.isLatestBid },
      ])}
      dark={tailwind([
        "border-gray-800",
        { "bg-gray-800": props.isLatestBid, "bg-gray-900": !props.isLatestBid },
      ])}
      ...
      <View style={tailwind("flex flex-row justify-between mb-2 items-center")}>
        <ThemedView
          ...
          light={tailwind([
            "text-white",
            {
              "bg-blue-500": props.isLatestBid,
              "bg-gray-400": !props.isLatestBid,
            },
          ])}
          dark={tailwind([
            "text-black",
            {
              "bg-darkblue-500": props.isLatestBid,
              "bg-gray-500": !props.isLatestBid,
            },
          ])}
        >
          <ThemedText
...
function EmptyBidHistory(props: {
  minNextBid: string;
  displaySymbol: string;
}): JSX.Element {
  return (
    <View
      style={tailwind("mt-24 flex items-center")}
      testID="empty_bid_history"
    >
      <ThemedIcon
