          ...
          >
            <NumberRow
              ...
              rhsUsdAmount={getTokenPrice(
                source.symbol,
                new BigNumber(amountToWithdraw),
                false
              )}
            />
            <NumberRow
    ...
    >
      <Slider
        ...
        onSlidingComplete={(val) =>
          props.onChange(new BigNumber(val).toFixed(8))
        }
      ...
      />
      <View style={tailwind("flex flex-row justify-between mt-1")}>
        <ThemedText
...
function TokenIcon(props: { icon: string }): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <ThemedText style={tailwind("ml-1")}>{props.icon}</ThemedText>
    </ThemedView>
  );
}
