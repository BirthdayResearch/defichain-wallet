...
const oceanIsDownContent: AnnouncementData[] = [
  {
    lang: {
      ...
      "zh-Hans":
        "我们目前正在调查 Ocean API 的连接问题。在 DeFiChain 状态页面上查看更多详细信息。",
      "zh-Hant":
        "我們目前正在調查 Ocean API 的連接問題。 在 DeFiChain 狀態頁面上查看更多詳細信息。",
...
export const blockChainIsDownContent: AnnouncementData[] = [
  {
    lang: {
      ...
      "zh-Hans":
        "我们目前正在调查区块链上的同步化问题。前往 DeFiChain Status 页面了解更多状态详情。",
      "zh-Hant":
        "我們目前正在調查區塊鏈上的同步化問題。前往 DeFiChain Status 頁面了解更多狀態詳情。",
...
export function useDeFiChainStatus(): {
  ...
  const [oceanStatusAnnouncement, setOceanStatusAnnouncement] = useState<
    AnnouncementData[] | undefined
  >();
  const [blockchainStatusAnnouncement, setBlockchainStatusAnnouncement] =
    useState<AnnouncementData[] | undefined>();
