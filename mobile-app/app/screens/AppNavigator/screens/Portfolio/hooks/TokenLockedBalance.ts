 ...
 */
export function useTokenLockedBalance({
  displaySymbol,
  denominationCurrency,
}: {
  displaySymbol?: string;
  denominationCurrency: string;
}): Map<string, LockedBalance> | LockedBalance | undefined {
  ...
  const computeLockedAmount = useCallback(() => {
    ...
    vaults.forEach((vault) => {
      ...
      vault.collateralAmounts.forEach((collateral) => {
        const token = clone(lockedBalance.get(collateral.displaySymbol)) ?? {
  ...
  return displaySymbol === undefined
    ? lockedBalance
    : lockedBalance?.get(displaySymbol);
}
...
export function useTokenBreakdownPercentage({
  available,
  locked,
}: {
  available: BigNumber;
  ...
  const availablePercentage = available
    .dividedBy(available.plus(locked))
    .multipliedBy(100);
  const lockedPercentage = locked
    .dividedBy(available.plus(locked))
    .multipliedBy(100);
  return {
    availablePercentage: availablePercentage.isNaN()
      ? new BigNumber(0)
      : availablePercentage,
    lockedPercentage: lockedPercentage.isNaN()
      ? new BigNumber(0)
      : lockedPercentage,
  };
}
