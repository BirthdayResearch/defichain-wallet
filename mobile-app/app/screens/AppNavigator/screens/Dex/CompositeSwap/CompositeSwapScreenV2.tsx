...
import { SlippageToleranceCard } from "./components/SlippageToleranceCard";
...
export function CompositeSwapScreenV2({ route }: Props): JSX.Element {
  ...
  // component UI state
  const { control, formState, setValue, getValues, trigger, watch } = useForm<{
    tokenA: string;
    tokenB: string;
  }>({ mode: "onChange" });
  ...
  const amountInUSDValue = useCallback(
  ...
  function showToast(type: AmountButtonTypes): void {
    ...
    const toastMessage = isMax
      ...
      : "{{percent}} of available {{token}} entered";
    const toastOption = {
      token: selectedTokenA?.displaySymbol,
      percent: type,
    };
        ...
        <View style={tailwind("my-8")}>
          <View>
                ...
                <View style={tailwind("w-6/12 mr-2")}>
                  <Controller
                    ...
                    rules={{
                      ...
                      pattern: /^\d*\.?\d*$/,
                      max:
                        selectedTokenA?.amount !== undefined
                          ? BigNumber.max(selectedTokenA.amount, 0).toFixed(8)
                          : new BigNumber(0).toFixed(8),
                      validate: {
            ...
            <TransactionCard
              maxValue={
                selectedTokenA != null
                  ? new BigNumber(getMaxAmount(selectedTokenA))
                  : new BigNumber(0)
              }
          ...
          >
            ...
            >
              {translate("screens/CompositeSwapScreen", "I WANT {{token}}", {
                token:
                  selectedTokenB != null ? selectedTokenB.displaySymbol : "",
              })}
            </ThemedTextV2>
            <View
              style={tailwind(
            ...
            >
              <WantSwapRow
                ...
                tokenUsdAmount={amountInUSDValue(
                  selectedTokenB ?? undefined,
                  tokenBAmount
                )}
              />
