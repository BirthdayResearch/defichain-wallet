        ...
        <SlippageButton
          ...
          icon={
            <ThemedIcon
              ...
              style={tailwind('mr-1')}
              dark={tailwind(`${isCustomSlippage ? 'text-darkprimary-500' : ''}`)}
              light={tailwind(`${isCustomSlippage ? 'text-primary-500' : ''}`)}
            />
          }
...
function SlippageButton ({ onPress, isActive, label, icon }: { onPress: () => void, isActive: boolean, label: string, icon?: ReactElement }): JSX.Element {
  ...
  const activeStyle = 'bg-primary-50 border-primary-50 text-primary-500'
  return (
    <View style={tailwind('mr-1.5 mt-2')}>
      ...
      >
        {icon}
        <ThemedText
          dark={tailwind(`${isActive ? 'text-darkprimary-500 font-semibold' : ''}`)}
          light={tailwind(`${isActive ? 'text-primary-500 font-semibold' : ''}`)}
