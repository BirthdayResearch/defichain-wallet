...
export function WantInfoText({
  tokenAAmount,
  selectedTokenA,
  isReservedUtxoUsed,
}: WantInfoTextProps): JSX.Element {
  const { infoText, infoTextThemedProps } = useMemo(() => {
    let infoText = "";
    let themedProps;

    if (
      new BigNumber(tokenAAmount).isGreaterThan(selectedTokenA?.amount ?? 0)
    ) {
  ...
  return <ThemedTextV2 {...infoTextThemedProps}>{infoText}</ThemedTextV2>;
}
