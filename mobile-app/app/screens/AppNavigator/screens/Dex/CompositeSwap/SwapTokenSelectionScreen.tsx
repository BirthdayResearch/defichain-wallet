...
import { ThemedFlatListV2, ThemedTextV2, ThemedTouchableOpacityV2 } from '@components/themed'
      ...
      ListHeaderComponent={
        <View style={tailwind('flex-col mt-8')}>
          <SearchInputV2
            ...
            placeholder={translate('screens/SwapTokenSelectionScreen', 'Search tokens')}
            containerStyle={
  ...
  >
    {translate('screens/SwapTokenSelectionScreen', listType === TokenListType.From ? 'AVAILABLE TOKENS' : 'AVAILABLE FOR SWAP')}
            ...
            : (
              ...
              >
                {translate('screens/SwapTokenSelectionScreen', debouncedSearchTerm.trim() === '' ? 'Search with token name' : 'Search results for “{{searchTerm}}”', { searchTerm: debouncedSearchTerm })}
              </ThemedTextV2>
            )}
        </View>
      }
...
interface TokenItemProps {
  item: SelectionToken
  onPress: any
  getTokenPrice: (symbol: string, amount: BigNumber, isLPS?: boolean | undefined) => BigNumber
...
const TokenItem = ({
  ...
  const activePrice = getTokenPrice(item.token.symbol, new BigNumber('1'), item.token.isLPS)
  return (
    <ThemedTouchableOpacityV2
      style={tailwind('flex flex-row p-5 mb-2 border-0 rounded-lg-v2 items-center justify-between')}
    ...
    >
      ...
      <View style={tailwind('flex-1 flex-wrap flex-col items-end')}>
        <NumberFormat
          ...
          renderText={value =>
            <ThemedTextV2
              style={tailwind('w-full flex-wrap font-semibold-v2 text-xs text-right')}
...
function filterTokensBySearchTerm (tokens: SelectionToken[], searchTerm: string, isFocused: boolean): SelectionToken[] {
