...
function ButtonGroupItem (props: ButtonGroupItemProps): JSX.Element {
  return (
    <TouchableOpacity
      ...
      style={props.customButtonGroupStyle ?? [tailwind('px-2'), { width: `${props.width.toFixed(2)}%` }]}
      testID={`${props.testID}${props.isActive ? '_active' : ''}`}
    >
      <View
        style={props.customButtonGroupStyle ?? [tailwind(['break-words justify-center pt-2.5 pb-4 border-brand-v2-500', { 'border-b-2 border-brand-v2-500': props.isActive }])]}
      >
        <ThemedTextV2
          light={tailwind({ 'text-brand-v2-500': props.isActive, 'text-mono-light-v2-900': !props.isActive })}
          dark={tailwind({ 'text-brand-v2-500': props.isActive, 'text-mono-dark-v2-900': !props.isActive })}
          style={props.labelStyle ?? tailwind('font-semibold-v2 text-sm text-center')}
        >
          {props.label}
        </ThemedTextV2>
      </View>
