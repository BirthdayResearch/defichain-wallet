...
export function PrivacyLockContextProvider(
  ...
  const [biometricHardwares, setBiometricHardwares] = useState<
    AuthenticationType[]
  >([]);
  ...
  const [isDevicePersistenceLoaded, setIsDevicePersistenceLoaded] =
  ...
  const context: PrivacyLockContextI = {
    ...
    prompt: async (options) => {
      if (
        !isDeviceProtected ||
        !(isPrivacyLock !== undefined && isPrivacyLock)
      ) {
        return;
      }
      ...
      if (Platform.OS === "ios") {
        switch (securityLevel) {
          ...
          case SecurityLevel.BIOMETRIC:
            if (
              biometricHardwares.includes(AuthenticationType.FACIAL_RECOGNITION)
            ) {
              return "Face ID";
            } else if (
              biometricHardwares.includes(AuthenticationType.FINGERPRINT)
            ) {
              return "Touch ID";
            } else {
  ...
  if (
    isPrivacyLock === undefined ||
    !(isLocalAuthLoaded && isDevicePersistenceLoaded)
  ) {
    return null;
  }
  return (
