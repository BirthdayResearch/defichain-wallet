...
export function useDisplayUtxoWarning(): {
  ...
  const getDisplayUtxoWarningStatus = useCallback(
    (amountInDFI: BigNumber, displaySymbol: string) => {
      if (
        new BigNumber(amountInDFI).isNaN() ||
        new BigNumber(amountInDFI).isZero() ||
        displaySymbol !== "DFI"
      ) {
        return false;
      }
