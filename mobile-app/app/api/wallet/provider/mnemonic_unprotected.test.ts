...
describe("getMnemonicHdNodeProvider", () => {
  ...
  it("should get provider (abandon x23)", async () => {
    ...
    expect(pubKey.toString("hex")).toStrictEqual(
      "03f85401f5aa4e9ed831120a22b8835137404755b30c59109c18c706b2549f7951"
    );
    expect(privKey.toString("hex")).toStrictEqual(
...
describe("addMnemonicHdNodeProvider", () => {
  it("should set mnemonic (abandon x23)", async () => {
    ...
    expect(data).toStrictEqual({
      ...
      raw: {
        ...
        privKey:
          "5237699220ac42181ec13168534b56aaad0b812ef09dc6427660ae89bbebaa7c",
        chainCode:
          "276c385797e9a2018cbc10e769f137198dd94719670ec0e5df3ea69385a33229",
      },
    });
  });
  ...
  it("should set mnemonic (void come effort ...)", async () => {
    const words =
      "void come effort suffer camp survey warrior heavy shoot primary clutch crush open amazing screen patrol group space point ten exist slush involve unfold".split(
        " "
      );

    expect(
      MnemonicUnprotected.toData(words, EnvironmentNetwork.LocalPlayground)
    ).toStrictEqual({
      version: "v1",
      type: WalletType.MNEMONIC_UNPROTECTED,
      raw: {
        words,
        privKey:
          "f97bae97aa293a5d5d62765684efe281e783aa927899c97c61a9ead3953286c3",
        chainCode:
          "67537da3836a1566399151c71e5e4e92e93386921748701bb66417cbb663c799",
      },
    });
  });
});
