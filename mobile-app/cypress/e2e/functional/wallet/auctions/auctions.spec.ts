...
context("Wallet - Auctions", () => {
  ...
  it("should show liquidated vault in auctions", () => {
    cy.getByTestID("bottom_tab_auctions").click();
    cy.getByTestID("batch_0_dTU10").should("exist");
  });

  it("should display important auction details", () => {
  ...
  it("should be able to quick/place bid", () => {
    ...
    runIfAuctionsIsAvailable(() => {
      ...
      cy.getByTestID("text_total_auction_value")
        .invoke("text")
        .then((text) => {
      ...
      cy.getByTestID("total_auction_value")
        .invoke("text")
        .then((text) => {
          checkValueWithinRange("2000.00", text.replace("$", ""));
        });
  ...
  describe("Min. Next Bid", () => {
    ...
    function validateLoanTokenUSDValue(
      tokenTestID: string,
      usdTestID: string
    ): void {
      cy.wrap<LoanVaultLiquidated[]>(whale.poolpairs.listDexPrices("USDT"), {
        timeout: 20000,
      }).then((response): void => {
        cy.getByTestID(tokenTestID)
          .invoke("text")
          .then((text: string) => {
            const minNextBid = text.replace(" dTU10", "");
            cy.getByTestID(usdTestID)
              .invoke("text")
              .then((actualUSD: string) => {
                const price = response.dexPrices.TU10;
                const estimatedUSD = new BigNumber(minNextBid).times(
    ...
    it("should display USD values in batch card", () => {
      cy.go("back");
      runIfAuctionsIsAvailable(() => {
        validateLoanTokenUSDValue(
          "batch_0_min_next_bid",
          "batch_0_min_next_bid_usd"
        );
      });
    });

    it("should display USD values in quick bid bottom sheet in auction home screen", () => {
      runIfAuctionsIsAvailable(() => {
        cy.getByTestID("batch_card_0_quick_bid_button").click();
        validateLoanTokenUSDValue(
          "quick_bid_min_next_bid",
          "quick_bid_min_next_bid_usd"
        );
        validateLoanTokenUSDValue(
          "text_current_balance",
          "quick_bid_current_balance_usd"
        );
        cy.getByTestID("quick_bid_close_button").click();
      });
    });

    it("should display USD values in place bid screen", () => {
      runIfAuctionsIsAvailable(() => {
        cy.getByTestID("batch_card_0_place_bid_button").click();
        validateLoanTokenUSDValue(
          "text_min_next_bid",
          "place_bid_min_next_bid_usd"
        );
        cy.go("back");
      });
    });

    it("should display USD values in auction details screen", () => {
      runIfAuctionsIsAvailable(() => {
        cy.getByTestID("batch_card_0").click();
        validateLoanTokenUSDValue(
          "auction_detail_min_next_bid",
          "auction_detail_min_next_bid_usd"
        );
      });
    });

    it("should display USD values in quick bid bottom sheet in auction details screen", () => {
      runIfAuctionsIsAvailable(() => {
        cy.getByTestID("auction_details_quick_bid_button").click();
        validateLoanTokenUSDValue(
          "quick_bid_min_next_bid",
