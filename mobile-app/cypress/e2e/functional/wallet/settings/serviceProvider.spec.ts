...
defichainUrlEnvs.forEach((defichainUrlEnv) => {
  context("Wallet - Settings - Service Provider", () => {
    ...
    context(
      `Wallet - Settings - Service Provider ${defichainUrlEnv}`,
      { testIsolation: false },
      () => {
        before(() => {
        ...
        it(`should display default on first app load on ${defichainUrlEnv}`, () => {
          cy.getByTestID("header_network_name")
            ...
            .then((network: string) => {
              ...
              cy.getByTestID(
                "setting_navigate_service_provider_value",
              ).contains("Default");
        ...
        it(`should type invalid custom provider URL on ${defichainUrlEnv}`, () => {
          ...
          cy.getByTestID("DVM_endpoint_url_input_error").contains(
            "Invalid URL",
          );
          cy.getByTestID("button_submit").should("have.attr", "aria-disabled");
        });
...
context(
  "Wallet - Settings - Service Provider Feature Gated",
  { testIsolation: false },
  () => {
    before(() => {
      cy.clearLocalStorage();
