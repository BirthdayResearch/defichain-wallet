...
function validatePriceSelectionAndSummaryBasedOnPercentage (): void {
  ...
  cy.getByTestID('pricerate_value_0_rhsUsdAmount').invoke('text').then(value => {
  ...
  cy.getByTestID('pricerate_value_1_rhsUsdAmount').invoke('text').then(value => {
    checkValueWithinRange(value, '2,500.00')
  })
  ...
  cy.getByTestID('tokens_remove_amount_input').should('have.value', '10.00000000')
  cy.getByTestID('pricerate_value_0').invoke('text').then(value => {
...
context('Wallet - DEX - View pool share information', () => {
  ...
  it('clicking view pool share should open details modal and able to close modal', () => {
    ...
    cy.getByTestID('view_pool_details_title').should('have.text', `${tokenA}-${tokenB}`)
    cy.getByTestID('lp_tokens_value').should('exist').contains('10.00000000')
    cy.getByTestID('lp_tokens_percentage_amount').should('exist')

    cy.getByTestID(`token_in_${tokenA}_value`).invoke('text').then(value => {
      checkValueWithinRange(value, '100.00000000')
    })
    cy.getByTestID(`token_in_${tokenA}_value_rhsUsdAmount`).invoke('text').then(value => {
      checkValueWithinRange(value, '10,000.00')
    })
    cy.getByTestID(`token_in_${tokenB}_value`).invoke('text').then(value => {
      checkValueWithinRange(value, '0.10000000')
    })
    cy.getByTestID(`token_in_${tokenB}_value_rhsUsdAmount`).invoke('text').then(value => {
      checkValueWithinRange(value, '10,000.00')
    })
...
context('Wallet - DEX - Remove Liquidity', () => {
  ...
  it('should show the available LP tokens equal to their current liqudity pool', () => {
    cy.getByTestID(`token_balance_${tokensPair}`).should('exist').contains('10.00000000')
  })
...
context('Wallet - DEX - Remove Liquidity Confirm Txn', () => {
  ...
  it('Should be able to remove liquidity', function () {
    ...
    cy.getByTestID('pricerate_value_0').invoke('text').then(valueA => {
      ...
      cy.getByTestID('pricerate_value_1').invoke('text').then(valueB => {
        ...
        cy.getByTestID('confirm_pricerate_value_1_label').contains(`${tokenB} to receive`)
  ...
  it('should be able to remove correct liquidity when user cancel a tx and updated some inputs', function () {
    ...
    // Prices section info
    cy.getByTestID('confirm_pricerate_value_0_label').contains(`${tokenA} to receive`)
    cy.getByTestID('confirm_pricerate_value_0').invoke('text').then((value) => {
      checkValueWithinRange(value, '50.00000000')
    })
    cy.getByTestID('confirm_pricerate_value_0_rhsUsdAmount').invoke('text').then((value) => {
      checkValueWithinRange(value, '5,000.00')
    })
    cy.getByTestID('confirm_pricerate_value_1_label').contains(`${tokenB} to receive`)
    cy.getByTestID('confirm_pricerate_value_1').invoke('text').then((value) => {
      checkValueWithinRange(value, '0.50000000')
    })
    cy.getByTestID('confirm_pricerate_value_1_rhsUsdAmount').invoke('text').then((value) => {
      checkValueWithinRange(value, '5,000.00')
    })
