...
context("Wallet - Pool Pair Rewards", () => {
  ...
  context("Wallet with LP Tokens", () => {
    ...
    it("should create Wallet B with LP tokens", () => {
      cy.createEmptyWallet(true);
    });

    it("should not have any DFI tokens", () => {
      cy.wait(3000);
      cy.getByTestID("bottom_tab_portfolio").click();
      cy.getByTestID("dfi_total_balance_amount").contains("0.00000000");
    });

    it("should receive LP tokens DFI tokens and receive rewards", () => {
      cy.sendDFItoWallet().sendTokenToWallet(["BTC-DFI"]).wait(10000);
