...
context("Wallet - Send - Max Values", () => {
  let whale: WhaleApiClient;
...
context("Wallet - Send - with Conversion", () => {
  let whale: WhaleApiClient;
...
context("Wallet - Send - Switch token", () => {
  before(() => {
    cy.createEmptyWallet(true);
    cy.sendDFItoWallet()
...
context("Wallet - Send - Address book", () => {
...
context("(evm <> dvm) Wallet - Send", () => {
  ...
  it("should be able to send tokens from EVM to other DVM address", () => {
    ...
    cy.getByTestID("verified_address").should("have.text", "Verified ");
    cy.getByTestID("amount_input").clear().type(amt.toString());
    cy.getByTestID("button_confirm_send_continue").should(
      "not.have.attr",
      "disabled",
