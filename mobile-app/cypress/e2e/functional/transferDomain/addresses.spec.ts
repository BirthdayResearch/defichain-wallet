context("Portfolio - Send - Address Book", { testIsolation: false }, () => {
  ...
  describe(
    "Whitelisted and Your Addresses tab",
    { testIsolation: false },
    () => {
      before(() => {
        cy.clearAllCookies();
      ...
      it("(dvm) Whitelisted - should display evm tag for evm addresses", () => {
        ...
        cy.getByTestID("address_row_label_1_WHITELISTED_EVM_tag").should(
          "exist",
        );
      });
...
context("Portfolio", { testIsolation: false }, () => {
  before(() => {
    cy.clearAllCookies();
    cy.clearAllLocalStorage();
    cy.setFeatureFlags(["evm"]).wait(1000);
